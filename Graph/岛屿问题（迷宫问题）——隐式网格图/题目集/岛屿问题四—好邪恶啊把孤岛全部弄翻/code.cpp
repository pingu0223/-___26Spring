...
void BFS(vector<vector<int>>&matrix,vector<vector<bool>>&v,int i,int j){
    int flag=0;
    int r_l=matrix.size();int c_l=matrix[0].size();
    if(i<0||i>=r_l||j<0||j>=c_l) return ;
    if(v[i][j]||matrix[i][j]==0) return ;
    ...
    if(flag==1) return;
        ...
        int n=result.size();
        while(!result.empty()){
            pair<int,int> curr=result.front();result.pop();
